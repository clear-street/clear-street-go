...
func TestV1WatchlistItemAddWatchlistItem(t *testing.T) {
	...
	_, err := client.V1.Watchlists.Items.AddWatchlistItem(
		...
		clearstreet.V1WatchlistItemAddWatchlistItemParams{
...
func TestV1WatchlistItemDeleteWatchlistItem(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.V1.Watchlists.Items.DeleteWatchlistItem(
		...
		clearstreet.V1WatchlistItemDeleteWatchlistItemParams{
