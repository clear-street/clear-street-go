...
// trading_account_ids.
//
// ActiveV1OmniAIEntitlementAgreementService contains methods and other services
// that help with interacting with the clear-street API.
//
// Note, unlike clients, this service does not read variables from the environment
// automatically. You should not instantiate this service directly, and instead use
// the [NewActiveV1OmniAIEntitlementAgreementService] method instead.
type ActiveV1OmniAIEntitlementAgreementService struct {
	options []option.RequestOption
}

// NewActiveV1OmniAIEntitlementAgreementService generates a new service that
// applies the given options to each request. These options are applied after the
// parent client's options (if there is one), and before any request-specific
// options.
func NewActiveV1OmniAIEntitlementAgreementService(opts ...option.RequestOption) (r ActiveV1OmniAIEntitlementAgreementService) {
	r = ActiveV1OmniAIEntitlementAgreementService{}
	r.options = opts
	return
}

// List current signable entitlement agreements for consent UX.
func (r *ActiveV1OmniAIEntitlementAgreementService) ListEntitlementAgreements(ctx context.Context, opts ...option.RequestOption) (res *ActiveV1OmniAIEntitlementAgreementListEntitlementAgreementsResponse, err error) {
	opts = slices.Concat(r.options, opts)
	path := "active/v1/omni-ai/entitlement-agreements"
...
type ActiveV1OmniAIEntitlementAgreementListEntitlementAgreementsResponse struct {
...
// Returns the unmodified JSON received from the API
func (r ActiveV1OmniAIEntitlementAgreementListEntitlementAgreementsResponse) RawJSON() string {
	return r.JSON.raw
}
func (r *ActiveV1OmniAIEntitlementAgreementListEntitlementAgreementsResponse) UnmarshalJSON(data []byte) error {
	return apijson.UnmarshalRoot(data, r)
}
