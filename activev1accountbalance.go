...
type AccountBalances struct {
	...
	Balance RegTBalance `json:"balance,required"`
	// Realized profit or loss since start of day.
	DailyRealizedPnl string `json:"daily_realized_pnl,required"`
	// Total profit or loss since start of day.
	DailyTotalPnl string `json:"daily_total_pnl,required"`
	// Total unrealized profit or loss across all positions relative to prior close.
	DailyUnrealizedPnl string `json:"daily_unrealized_pnl,required"`
	...
	// "FUTURES_TOT_EQ".
	MarginType MarginType `json:"margin_type,required"`
	// Timestamp for the start-of-day values
	SodAsof APITimestamp `json:"sod_asof,nullable" format:"date-time"`
...
type RegTBalance struct {
	...
	BuyingPower string `json:"buying_power,required"`
	// Currency identifier for all monetary values
	Currency string `json:"currency,required"`
	// Day-trading buying power.
	DaytradingBuyingPower string `json:"daytrading_buying_power,required"`
	// The total equity in the account (market value of all assets minus liabilities)
	Equity string `json:"equity,required"`
	// The total market value of all long positions
	LongMarketValue string `json:"long_market_value,required"`
	// Margin requirement for trade-date balances.
	MaintenanceMargin string `json:"maintenance_margin,required"`
	...
	SettledCash string `json:"settled_cash,required"`
	// The total market value of all short positions (represented as a positive value)
	ShortMarketValue string `json:"short_market_value,required"`
	// Start-of-day cash balance.
	SodCash string `json:"sod_cash,required"`
	// Start-of-day day-trading buying power.
	SodDaytradingBuyingPower string `json:"sod_daytrading_buying_power,required"`
	// Start-of-day equity based on cash and positions.
	SodEquity string `json:"sod_equity,required"`
	// Start-of-day long position market value (ex-cash).
	SodLongMarketValue string `json:"sod_long_market_value,required"`
	// Start-of-day margin excess.
	SodMarginExcess string `json:"sod_margin_excess,required"`
	// Start-of-day margin requirement.
	SodMarginRequirement string `json:"sod_margin_requirement,required"`
	// Start-of-day Regulation T buying power.
	SodRegTBuyingPower string `json:"sod_reg_t_buying_power,required"`
	// Start-of-day short position market value (ex-cash).
	SodShortMarketValue string `json:"sod_short_market_value,required"`
	// Aggregated cash value.
	TradeCash string `json:"trade_cash,required"`
	// Trade-date unsettled cash credits.
	UnsettledCashCredits string `json:"unsettled_cash_credits,required"`
	// Trade-date unsettled cash debits.
	UnsettledCashDebits string `json:"unsettled_cash_debits,required"`
	// JSON contains metadata for fields, check presence with [respjson.Field.Valid].
	JSON struct {
...
type ActiveV1AccountBalanceGetAccountBalancesResponse struct {
	// Represents the balance details for a trading account
	Data AccountBalances `json:"data,required"`
	// JSON contains metadata for fields, check presence with [respjson.Field.Valid].
	JSON struct {
		Data        respjson.Field
		ExtraFields map[string]respjson.Field
