...
//
// ActiveV1InstrumentFundamentalService contains methods and other services that
// help with interacting with the clear-street API.
//
// Note, unlike clients, this service does not read variables from the environment
// automatically. You should not instantiate this service directly, and instead use
// the [NewActiveV1InstrumentFundamentalService] method instead.
type ActiveV1InstrumentFundamentalService struct {
	options []option.RequestOption
}

// NewActiveV1InstrumentFundamentalService generates a new service that applies the
// given options to each request. These options are applied after the parent
// client's options (if there is one), and before any request-specific options.
func NewActiveV1InstrumentFundamentalService(opts ...option.RequestOption) (r ActiveV1InstrumentFundamentalService) {
	r = ActiveV1InstrumentFundamentalService{}
	r.options = opts
	return
}

// Retrieves supplemental fundamentals and company profile data for an instrument.
func (r *ActiveV1InstrumentFundamentalService) GetInstrumentFundamentals(ctx context.Context, id string, opts ...option.RequestOption) (res *ActiveV1InstrumentFundamentalGetInstrumentFundamentalsResponse, err error) {
	opts = slices.Concat(r.options, opts)
	if id == "" {
		err = errors.New("missing required id parameter")
		return nil, err
	}
	path := fmt.Sprintf("active/v1/instruments/%s/fundamentals", id)
...
type ActiveV1InstrumentFundamentalGetInstrumentFundamentalsResponse struct {
	// Supplemental fundamentals and company profile data for an instrument.
...
// Returns the unmodified JSON received from the API
func (r ActiveV1InstrumentFundamentalGetInstrumentFundamentalsResponse) RawJSON() string {
	return r.JSON.raw
}
func (r *ActiveV1InstrumentFundamentalGetInstrumentFundamentalsResponse) UnmarshalJSON(data []byte) error {
	return apijson.UnmarshalRoot(data, r)
}
